...
module.exports = class PusherPubSubGateway {
  ...
  async subscribe(channelName, eventName, callback) {
    console.log(`i am subscribed to ${channelName}, ${eventName}`);
  ...
  connect() {
    const error = new Errors.PubSubConnectionError(
      "Error establishing web socket connection to signaling server"
    );
