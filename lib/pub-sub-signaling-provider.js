...
module.exports = class PubSubSignalingProvider {
  ...
  async send(signal) {
    ...
    // const { ok, status, body } = { ok: true, status: 200, body: "todo" };
    if (status === 401) {
      throw new Errors.InvalidAuthenticationTokenError(
        "The provided authentication token is invalid"
      );
    } else if (!ok) {
      throw new Errors.PubSubConnectionError(
        "Error signalling peer: " + body.message
      );
    }
  }
