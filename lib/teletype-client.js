...
module.exports = class TeletypeClient {
  constructor({
    restGateway,
    pubSubGateway,
    connectionTimeout,
    ...
    this.pubSubGateway = new PusherPubSubGateway({
      key: pusherKey,
      options: pusherOptions
    });
    this.connectionTimeout = connectionTimeout || 5000;
    this.tetherDisconnectWindow =
      tetherDisconnectWindow || DEFAULT_TETHER_DISCONNECT_WINDOW;
  ...
  async initialize() {
    ...
    if (ok && body.version > LOCAL_PROTOCOL_VERSION) {
      throw new Errors.ClientOutOfDateError(
        `This version teletype-client is out of date. The local version is ${LOCAL_PROTOCOL_VERSION} but the remote version is ${
          body.version
        }.`
      );
    }
  ...
  async signIn(oauthToken) {
    ...
    if (response.ok) {
      ...
      const message =
        "Authentication failed with message: " + response.body.message;
      throw new Errors.UnexpectedAuthenticationError(message);
    }
  }
  ...
  async createPortal() {
    let result;
    try {
      result = await this.restGateway.post("/portals", {
        hostPeerId: this.getClientId()
      });
    } catch (error) {
      throw new Errors.PortalCreationError(
        "Could not contact server to create your portal"
      );
    }
    ...
    if (result.ok) {
      ...
      throw new Errors.PortalCreationError(
        "A server error occurred while creating your portal"
      );
    }
  }
  ...
  async joinPortal(id) {
    ...
    try {
      ...
      throw new Errors.PortalJoinError(
        "Could not contact server to join the portal"
      );
    }
    ...
    if (!this.clientId) {
      ...
      for (const networkInterfaceName in networkInterfaces) {
        ...
        if (
          ...
          networkAddress.mac !== EMPTY_MAC_ADDRESS
        ) {
      ...
      if (macAddress) {
        ...
        const macAddressBytes = macAddress
          .split(":")
          .map(part => Buffer.from(part, "hex").readUInt8());
