...
module.exports = class PeerPool {
  constructor({
    ...
    this.peerIdentitiesById = new Map([[peerId, peerIdentity]]);
    this.disposed = false;
  ...
  async listen() {
    console.log("peerpool.listen() called");
    if (!this.listenPromise) {
      const timeoutError = new Errors.PubSubConnectionError(
  ...
  async connectTo(peerId) {
    if (this.peerId === peerId) {
      throw new Errors.PeerConnectionError(
        "Sorry. You can't connect to yourself this way. Maybe try meditation or a walk in the woods instead?"
      );
    }
