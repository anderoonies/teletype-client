...
const BufferProxy = require("./buffer-proxy").default;
const EditorProxy = require("./editor-proxy").default;
...
module.exports = class Portal {
  constructor({
    ...
    this.network = new StarOverlayNetwork({
      id,
      isHub: this.isHost,
      peerPool,
      connectionTimeout
    ...
    if (this.isHost) {
      ...
      this.disposables.add(
        this.router.onRequest(
          `/portals/${id}`,
          this.receiveSubscription.bind(this)
        )
      );
    } else {
  ...
  async join() {
    ...
    const rawResponse = await this.router.request({
      recipientId: this.hostPeerId,
      channelId: `/portals/${this.id}`
    });
    ...
    response
      .getEditorProxiesMetadataList()
      .forEach(editorProxyMetadataMessage => {
  ...
  createBufferProxy(props) {
    ...
    const bufferProxy = new BufferProxy(
      Object.assign(
        {
          id,
  ...
  async fetchBufferProxy(id) {
    const response = await this.router.request({
      recipientId: this.hostPeerId,
      channelId: `/buffers/${id}`
    });
    if (response.ok) {
      const bufferProxyMessage = Messages.BufferProxy.deserializeBinary(
        response.body
      );
  ...
  createEditorProxy(props) {
    ...
    const editorProxy = new EditorProxy(
      Object.assign(
        {
          id,
          siteId: this.siteId,
  ...
  async fetchEditorProxy(id) {
    ...
    const response = await this.router.request({
      recipientId: this.hostPeerId,
      channelId: `/editors/${id}`
    });
    if (!response.ok) return;

    const editorProxyMessage = Messages.EditorProxy.deserializeBinary(
      response.body
    );
    const bufferProxy = await this.findOrFetchBufferProxy(
      editorProxyMessage.getBufferProxyId()
    );
    if (bufferProxy) {
  ...
  deserializeEditorProxy(message) {
    ...
    const editorProxyMetadata = this.editorProxiesMetadataById.get(
      editorProxy.id
    );
  ...
  deserializeEditorProxyMetadata(message) {
    ...
    this.editorProxiesMetadataById.set(
  ...
  sendSubscriptionResponse(requestId) {
    ...
    this.activeEditorProxiesBySiteId.forEach((editorProxy, siteId) => {
      if (editorProxy) {
        ...
        response
          .getActiveEditorProxyIdsBySiteIdMap()
          .set(siteId, editorProxy.id);
      }
    });
    response.setActiveBufferProxiesList(
      Array.from(activeBufferProxiesById.values())
    );
    response.setActiveEditorProxiesList(
      Array.from(activeEditorProxiesById.values())
    );
  ...
  async receiveUpdate({ senderId, body }) {
    ...
    if (updateMessage.hasEditorProxyCreation()) {
      ...
      await this.receiveEditorProxySwitch(
        senderSiteId,
        updateMessage.getEditorProxySwitch()
      );
  ...
  receiveEditorProxyCreation(editorProxyCreationMessage) {
    this.deserializeEditorProxyMetadata(
  ...
  receiveTetherUpdate(tetherUpdate) {
    ...
    this.tethersByFollowerId.set(followerSiteId, {
      leaderId: leaderSiteId,
      state: tetherState
    });
  ...
  subscribeToEditorProxyChanges(editorProxy) {
    ...
    this.activeEditorProxySubscriptions = new CompositeDisposable();
    if (editorProxy) {
      this.activeEditorProxySubscriptions.add(
        editorProxy.onDidScroll(this.activeEditorDidScroll.bind(this))
      );
      this.activeEditorProxySubscriptions.add(
        editorProxy.onDidUpdateLocalSelections(
          this.activeEditorDidUpdateLocalSelections.bind(this)
        )
      );
      this.activeEditorProxySubscriptions.add(
        editorProxy.onDidUpdateRemoteSelections(
          this.activeEditorDidUpdateRemoteSelections.bind(this)
        )
      );
      this.activeEditorProxySubscriptions.add(
        editorProxy.bufferProxy.onDidUpdateText(
          this.activeEditorDidUpdateText.bind(this)
        )
      );
    }
  }
  ...
  activeEditorDidUpdateLocalSelections({ initialUpdate }) {
    ...
    if (!initialUpdate && this.resolveFollowState() === FollowState.RETRACTED) {
      const localCursorPosition = this.getLocalActiveEditorProxy().cursorPositionForSiteId(
        this.siteId
      );
      ...
      if (
        localCursorPosition &&
        leaderPosition &&
        !pointsEqual(localCursorPosition, leaderPosition)
      ) {
        this.extendTether();
      }
    }
  ...
  activeEditorDidUpdateRemoteSelections({
    selectionLayerIdsBySiteId,
    initialUpdate
  }) {
    const leaderDidChangeSelections = selectionLayerIdsBySiteId.has(
      this.resolveLeaderSiteId()
    );
  ...
  activeEditorDidUpdateText({ remote }) {
    if (this.resolveFollowState() === FollowState.RETRACTED) {
      if (remote) {
        this.delegate.updateTether(
          FollowState.RETRACTED,
          this.getLocalActiveEditorProxy(),
          this.resolveLeaderPosition()
        );
      } else {
  ...
  activeEditorDidScroll() {
    ...
    if (
      leaderPosition &&
      this.getLocalActiveEditorProxy().isScrollNeededToViewPosition(
        leaderPosition
      )
    ) {
      this.unfollow();
  ...
  retractOrDisconnectTether() {
    ...
    const leaderActiveEditorProxy = this.activeEditorProxyForSiteId(
      leaderSiteId
    );
    const leaderPositionIsInvisible =
      localActiveEditorProxy !== leaderActiveEditorProxy ||
      (leaderPosition &&
        localActiveEditorProxy.isScrollNeededToViewPosition(leaderPosition));

    const hasRecentlyPerformedLocalUpdate =
      Date.now() - this.lastLocalUpdateAt <= this.tetherDisconnectWindow;
  ...
  setFollowState(newState, newLeaderId) {
    ...
    this.tethersByFollowerId.set(this.siteId, {
  ...
  didChangeTetherState({
    oldResolvedState,
    oldResolvedLeaderId,
    newResolvedState,
    newResolvedLeaderId
  }) {
    const oldLeaderActiveEditorProxy = this.activeEditorProxiesBySiteId.get(
      oldResolvedLeaderId
    );
    const newLeaderActiveEditorProxy = this.activeEditorProxiesBySiteId.get(
      newResolvedLeaderId
    );
    ...
    if (newResolvedState === FollowState.RETRACTED) {
      ...
      console.log(oldLeaderActiveEditorProxy);
      this.activeEditorProxiesBySiteId.set(
        this.siteId,
        oldLeaderActiveEditorProxy
      );
      this.broadcastEditorProxySwitch(oldLeaderActiveEditorProxy);
      if (oldLeaderActiveEditorProxy)
        oldLeaderActiveEditorProxy.showSelections();
    }
  ...
  broadcastEditorProxyCreation(editorProxy) {
    ...
    editorProxyCreationMessage.setEditorProxyMetadata(
      editorProxy.getMetadata().serialize()
    );
    const updateMessage = new Messages.PortalUpdate();
