import {
  serializeRemotePosition,
  deserializeRemotePosition
...
export default class EditorProxy {
  static deserialize(message, props) {
    ...
    return new EditorProxy(
      Object.assign({ id, bufferProxy, selectionLayerIdsBySiteId }, props)
    );
  }
  ...
  constructor({
    ...
    this.subscriptions.add(
      this.bufferProxy.onDidUpdateMarkers(
        this.bufferProxyDidUpdateMarkers.bind(this)
      )
    );
    this.subscriptions.add(
  ...
  dispose() {
    ...
    if (this.isHost)
      this.router.notify({ channelId: `/editors/${this.id}/disposal` });
    this.emitDidDispose();
  }
  ...
  createLocalSelectionsLayer(selections) {
    ...
    selectionsUpdateMessage
      .getSelectionLayerIdsBySiteIdMap()
      .set(this.siteId, localSelectionsLayerId);
    ...
    this.router.notify({
      ...
      body: editorProxyUpdateMessage.serializeBinary()
    });
  ...
  updateSelections(selections = {}, options = {}) {
    ...
    if (this.selectionsVisible) {
      const localSelectionsLayerId = this.selectionLayerIdsBySiteId.get(
        this.siteId
      );
      this.bufferProxy.updateMarkers({
  ...
  cursorPositionForSiteId(siteId) {
    ...
    if (selectionIds.length > 0) {
      ...
      return lastSelection.reversed
        ? lastSelection.range.start
        : lastSelection.range.end;
    }
  }
  ...
  getLocalHiddenSelections() {
    ...
    const localHiddenSelectionsLayer = localLayers
      ? localLayers[this.localHiddenSelectionsLayerId]
      : null;
