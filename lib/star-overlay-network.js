...
module.exports = class StarOverlayNetwork {
  ...
  async connectTo(hubId) {
    ...
    const timeoutError = new Errors.NetworkConnectionError(
      "Connecting to the portal network timed out"
    );
  ...
  getMemberIdentity(peerId) {
    console.log("getting member identity for peerid");
    console.log(peerId);
    if (!this.memberIdentitiesById.has(peerId)) {
      ...
      process.nextTick(() => {
        const error = new Error(
          "Attempted to get identity for non-connected member"
        );
  ...
  receive({ senderId, message }) {
    ...
    if (networkMessage.hasStarJoinRequest()) {
      ...
      this.receiveLeaveNotification(
        message,
        networkMessage.getStarLeaveNotification()
      );
  ...
  receiveJoinRequest(rawMessage, connectionMessage) {
    ...
    console.log('i just got a join request!')
    ...
    joinNotificationMessage.setMemberIdentity(
      serializePeerIdentity(senderIdentity)
    );
    ...
    this.forwardBroadcast(
      senderId,
      notificationNetworkMessage.serializeBinary()
    );
  ...
  receiveJoinResponse(joinResponseMessage) {
    ...
    joinResponseMessage
      .getMemberIdentitiesByIdMap()
      .forEach((identityMessage, peerId) => {
        this.memberIdentitiesById.set(
          peerId,
          deserializePeerIdentity(identityMessage)
        );
  ...
  receiveJoinNotification(joinNotificationMessage) {
    ...
    const memberIdentity = deserializePeerIdentity(
      joinNotificationMessage.getMemberIdentity()
    );
