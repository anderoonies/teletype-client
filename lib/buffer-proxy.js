// import {
//   Document as CRDTDocument,
//   serializeOperation,
//   deserializeOperation
// } from "@atom/teletype-crdt";
const TeletypeCRDT = require('@atom/teletype-crdt');
...
export default class BufferProxy {
  static deserialize(message, props) {
    const id = message.getId();
    const uri = message.getUri();
    const operations = message.getOperationsList().map(TeletypeCRDT.deserializeOperation);
  ...
  constructor({
    ...
    // weird, whatever
    this.document = new TeletypeCRDT.Document.default({ siteId, text, history });
  ...
  dispose() {
    ...
    if (this.isHost)
      this.router.notify({ channelId: `/buffers/${this.id}/disposal` });
    this.emitDidDispose();
  }
  ...
  serialize() {
    ...
    bufferProxyMessage.setOperationsList(
      this.document.getOperations().map(TeletypeCRDT.serializeOperation)
    );
  ...
  receiveFetch({ requestId }) {
    this.router.respond({
      requestId,
      body: this.serialize().serializeBinary()
    });
  }
  ...
  receiveOperationsUpdate(operationsUpdateMessage) {
    const operations = operationsUpdateMessage
      .getOperationsList()
      .map(TeletypeCRDT.deserializeOperation);
  ...
  broadcastOperations(operations) {
    ...
    operationsUpdateMessage.setOperationsList(
      operations.map(TeletypeCRDT.serializeOperation)
    );
  ...
  integrateOperations(operations) {
    const { textUpdates, markerUpdates } = this.document.integrateOperations(
      operations
    );
